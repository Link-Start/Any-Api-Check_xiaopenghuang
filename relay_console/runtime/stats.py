...
from .db import (
    ensure_db,
    get_db_path,
    load_checkin_logs_payload,
    load_sites_payload,
    save_checkin_logs_payload,
    save_sites_payload,
)
...
def load_checkin_log() -> list:
    """加载签到日志"""
    ensure_db()
    logs = load_checkin_logs_payload()
...
def save_checkin_log(logs: list) -> bool:
    ...
    try:
        ensure_db()
        save_checkin_logs_payload(logs)
        return True
    except Exception as e:
        import sys
        print(f"保存签到日志失败: {e}", file=sys.stderr)
        return False
...
def load_stats() -> dict:
    """加载统计数据"""
    ensure_db()
    return {"sites": load_sites_payload()}
...
def save_stats(data: dict) -> bool:
    ...
    try:
        ensure_db()
        save_sites_payload(data.get("sites", []) if isinstance(data, dict) else [])
        return True
    except Exception as e:
        import sys
        print(f"保存统计数据失败: {e}", file=sys.stderr)
        return False
...
def create_site(
    ...
    return {
        ...
        "api_balance": balance,
        "api_balance_unit": balance_unit,
        "api_last_query_time": "",
        "account_balance": 0,
        "account_balance_unit": "USD",
        "account_last_query_time": "",
